...
const STEPS = ["Data Upload", "Year Correction", "Regression", "Clustering", "KNN Classification"];
const FORMATS = [
  { id: 1, label: "Format 1", desc: "Same form, different labels" },
  { id: 2, label: "Format 2", desc: "Years in different columns" },
  { id: 3, label: "Format 3", desc: "Year in rows, metrics in columns" },
...
// ── Step 4: Clustering ────────────────────────────────────────────────────────
function StepClustering({uploadResult, onDone}){
  const availableFeatures = uploadResult?.pivot_features?.length
    ? uploadResult.pivot_features
    : (uploadResult?.metrics||[]);
      ...
      {result&&(
        <div className="card">
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
            ...
            {/* Only show Proceed if there are partial-data countries to classify */}
...
function StepKNN({uploadResult, clusteringResult}){
  const availableFeatures = clusteringResult?.selectedFeatures
    || uploadResult?.pivot_features
    || uploadResult?.metrics
    || [];

  // Pre-populate from clustering if available, so user doesn't have to redo config
  ...
  // Sync feature list from clusteringResult when it arrives
  useEffect(()=>{
  ...
  // CSV download helper — no backend endpoint needed for KNN results
  const downloadCSV = () => {
  ...
  if(!clusteringResult)
    return(
      <div className="card">
        <Alert type="warn">
          KNN Classification requires clustering to be run first. Go back to Step 4 and
          complete clustering — then return here.
        </Alert>
      </div>
    );
  ...
  return(
    <>
      ...
      {/* ── Context banner — what clustering produced ── */}
      <div className="card">
      ...
      <div className="card">
        ...
        <div className="two-col">
          <div>
            <InputField label="Reference Year" type="number" value={knnYear}
              onChange={setKnnYear} min={1900} max={2100} step={1}/>
          </div>
          <div>
            <Label>K (Neighbours)</Label>
            <div className="slider-row" style={{marginTop:6}}>
              <input type="range" className="slider" min={1} max={10} step={1}
                value={nNeighbors} onChange={e=>setNNeighbors(Number(e.target.value))}/>
        ...
        {/* Feature selection — pre-filled from clustering */}
        <div className="card-title">Features</div>
        <Alert type="info">
          Pre-filled from your clustering configuration. Adjust only if needed —
          features must match those used during clustering.
        </Alert>
        ...
        {selectedFeatures.length<2&&(
          <Alert type="warn">Select at least 2 features to run KNN.</Alert>
        )}
        ...
        <div className="btn-row" style={{marginTop:16}}>
          <button className="btn btn-primary"
            ...
            {loading?<><Spinner/> Classifying…</>:"🤖 Run KNN Classification"}
          </button>
      ...
      {result&&(
        <div className="card">
          <div className="card-header">
            <div className="card-title" style={{marginBottom:0}}>Classification Results</div>
            <div style={{fontSize:11,color:"var(--muted)"}}>
          ...
          {result.classified===0
            ...
            :<>
                ...
                <StatBox label="Avg Confidence"
                  value={
                    result.knn_results?.length
                      ?(result.knn_results.reduce((s,r)=>s+r.confidence,0)/result.knn_results.length*100).toFixed(1)+"%"
                      :"—"
                  }
                  color="var(--info)"/>
              ...
              {/* Confidence colour coding legend */}
              ...
              {/* Enhanced results table with confidence colour */}
              <div className="tbl-wrap">
                <table className="tbl">
                  <thead>
                    <tr>
                    ...
                    {result.knn_results.map((r,i)=>{
                      ...
                      return(
                        <tr key={i}>
                          ...
                          <td>
                            <span style={{
                              padding:"2px 10px",borderRadius:3,fontSize:11,fontWeight:600,
    ...
    <>
      ...
      <div className="app">
        ...
        <main className="main">
          ...
          {activeStep===2&&(
            <StepRegression uploadResult={uploadResult}/>
          )}
          {activeStep===3&&(
            <StepClustering
              uploadResult={uploadResult}
              onDone={d=>{setClusteringResult(d);goNext();}}/>
          )}
          {activeStep===4&&(
            <StepKNN
              uploadResult={uploadResult}
