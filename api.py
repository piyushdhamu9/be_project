...
#         # Compute the pivot column names that clustering will actually use
#         # so the UI shows exactly the right feature options
#         pivot_features = []
...
#                 # Use the most recent year or 2023 if available
#                 sample_year = 2023 if 2023 in available_years else (max(available_years) if available_years else 2023)
...
#                 else:
#                     # Fallback: use all unique metrics
#                     pivot_features = df["metric"].unique().tolist()
#             except Exception as e:
#                 # Fallback: use all unique metrics
#                 print(f"Error computing pivot features: {e}")
#                 pivot_features = df["metric"].unique().tolist() if "metric" in df.columns else []
...
#     sheet_name: str = Form("Sheet1"),
# ):
#     """Return column names and first 10 rows from an uploaded file for preview and Format-1 mapping."""
#     contents = await file.read()
#     buf = BytesIO(contents)
#     try:
#         if file_type == "Excel":
...
#             raise HTTPException(
#                 status_code=400, 
#                 detail=f"Data missing required columns: {missing_cols}. Available columns: {df.columns.tolist()}"
#             )
        
#         # Get available years in the data
#         available_years = sorted(df["year"].dropna().unique().tolist())
        
#         # If requested ref_year is not in data, use the latest available year
#         if req.ref_year not in available_years:
#             req.ref_year = max(available_years) if available_years else 2023
...
#             raise HTTPException(
#                 status_code=400, 
#                 detail=f"No data found for year {req.ref_year}. Available years: {available_years}"
#             )
...
#         # Validate and filter selected features to only those that exist in the pivoted DataFrame
#         available_features = [col for col in pdf.columns if col not in ["country", "Country", "COUNTRY", "source", "assumption"]]
        
#         if not available_features:
#             raise HTTPException(
#                 status_code=400, 
#                 detail=f"No metrics/features found in pivoted data for year {req.ref_year}. PDF columns: {pdf.columns.tolist()}"
#             )
        
#         valid_features = [feat for feat in req.selected_features if feat in available_features]
        
#         if not valid_features:
#             raise HTTPException(
#                 status_code=400, 
#                 detail=f"Selected features not found in data. Available features: {available_features}"
#             )
        
#         # Use only valid features
#         selected_features = valid_features

#         missing_counts    = pdf[selected_features].isna().sum(axis=1)
...
#         std_data = scaler.fit_transform(complete_data[selected_features])
#         std_df   = pd.DataFrame(std_data, columns=selected_features)
...
#     """Debug endpoint to show data structure before clustering"""
#     session = sessions.get(session_id)
#     if not session:
#         raise HTTPException(status_code=404, detail="Session not found.")
...
#         },
#         "required_columns_present": all(col in df.columns for col in ["country", "metric", "year", "value"]),
#     }
...
#         if not pdf.empty:
#             available_features = [col for col in pdf.columns if col not in ["country", "source", "assumption"]]
#             diagnosis["available_features"] = available_features
#             diagnosis["pivot_preview"] = pdf.head(3).replace({np.nan: None}).to_dict(orient="records")
#     except Exception as e:
